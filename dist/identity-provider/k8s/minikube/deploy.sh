...
then
    kubectl --namespace=iam exec -i ${HYDRA_POD} -- /go/bin/hydra clients create --skip-tls-verify --id mangoOIDCClientId --name mangoOIDCClientId --secret mangoOIDCClientSecret --grant-types authorization_code,refresh_token,client_credentials,implicit,urn:ietf:params:oauth:grant-type:jwt-bearer --response-types token,code,id_token --allowed-scopes openid,offline,hydra.keys.get,hydra,idp.auth.password,https://apis.sugarcrm.com/auth/crm,profile,email,address,phone  --callbacks "http://login.sugarcrm.local/consumer,http://localhost/consumer,http://behat-tests-mango-oidc.idm-ns-localhost/?module=Users&action=OAuth2CodeExchange"
fi
