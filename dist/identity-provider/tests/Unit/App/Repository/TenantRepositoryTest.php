...
class TenantRepositoryTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testFindTenantByIdTenantFound(): void
    {
        $this->db->expects($this->once())->method('fetchAssoc')->with(
            'SELECT * FROM tenants WHERE id = ?',
            ['0000000001']
        )->willReturn(['id' => '0000000001', 'region' => 'us']);

        $result = $this->repository->findTenantById($this->tenantId);

        $this->assertEquals('0000000001', $result->getId());
        $this->assertEquals('us', $result->getRegion());
    }
}
