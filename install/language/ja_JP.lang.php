...
$mod_strings = array(
	...
	'ERR_CHECKSYS_CURL'					=> '見つかりませんでした。Sugar Scheduler は制限された機能を実行します。メールアーカイブサービスは実行されません。',
    ...
    'LBL_CHECKSYS_DB_SUPPORT_NOT_AVAILABLE' => 'データベースサポートが見つかりませんでした。 MySQL または MS SQLServer、Oracle、DB2　のいずれかのサポートされているデータベースタイプに必要なドライバーがあることを確認してください。PHP のバージョンに応じて、php.ini ファイルの拡張子をコメント解除する、または適切なバイナリファイルで再度コンパイルする必要があります。データベースサポートを有効にする方法については PHP マニュアルを参照してください。',
